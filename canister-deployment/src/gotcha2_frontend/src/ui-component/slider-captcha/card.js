...
// const Card = ({ text, fetchCaptcha, submitResponse, tempCreateRes }) => {
  const [key, setKey] = useState(Math.random());
  const [captcha, setCaptcha] = useState(false);
  const isMounted = useRef(false);

  const refreshCaptcha = () => {
    // console.log("tempCreateRes: ", tempCreateRes);
    // setTimeout(() => {
    //   if (!isMounted.current) return;
    //   setKey(Math.random());
    //   setCaptcha(tempCreateRes);
    // }, 300);

    // ===========================================================

    fetchCaptcha().then((newCaptcha) => {
      console.log("refreshCaptcha: ", newCaptcha);
      setTimeout(() => {
        if (!isMounted.current) return;
        setKey(Math.random());
